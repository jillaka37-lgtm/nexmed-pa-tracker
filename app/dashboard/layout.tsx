...
export default async function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="relative min-h-screen">
      ...
      />
      <div aria-hidden className="fixed inset-0 bg-gradient-to-br from-navy/95 via-navy/90 to-teal/20" />

      <div className="relative">
        {/* Same /dashboard URL for everyone — the shell (and its nav) is
            chosen by role. Staff get the unified Operations + CRM console;
            patients get their own account view. Keeps "one dashboard" true
            at the URL level. */}
        {profile?.role === "admin" ? (
          <StaffShell greeting={greeting}>{children}</StaffShell>
        ) : (
